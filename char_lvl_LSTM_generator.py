...
file = unidecode.unidecode(open("dataset/names.txt").read())
...
class Generator():
    ...
    def get_random_batch(self):
        start_idx = random.randint(0, len(file) - self.chunk_length)
        end_idx = start_idx + self.chunk_length + 1
    ...
    def generate(self, initial_string='A', prediction_len=100, temperature=0.85):
        ...
        for p in range(len(initial_string) - 1):
            _, (hidden, cell) = self.rnn(initial_input[p].view(1).to(device),
                                         hidden, cell)
    ...
    def train(self):
        ...
        writer = SummaryWriter(f'runs/names')
